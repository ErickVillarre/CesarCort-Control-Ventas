...
export default function AdminAccess() {
  ...
  const cargar = async () => {
    try {
      ...
      const rolesPayload = rolesRes?.data || [];

      setEmpleados(employeesRes.data || []);
      setRoles(Array.isArray(rolesPayload) ? rolesPayload : rolesPayload.roles || []);
      setRoleMatrix(Array.isArray(rolesPayload) ? [] : rolesPayload.matrix || []);
    } catch (error) {
  ...
  const matrixPermissions = useMemo(() => {
    const requiredPrefixes = ["users.", "roles.", "sales.", "clients.", "credits.", "inventory.", "cash.", "maintenance.", "reports.", "web_requests."];

    return Object.keys(roleMatrix[0]?.permissions || {}).filter((permission) =>
      requiredPrefixes.some((prefix) => permission.startsWith(prefix))
    );
  }, [roleMatrix]);
  ...
  return (
    <div className="space-y-5">
      ...
      {canManageUsers && roleMatrix.length > 0 && (
        <section className="overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-sm">
          <div className="border-b border-zinc-200 px-4 py-3 font-semibold">Matriz de roles y permisos</div>
              ...
              <thead className="bg-zinc-100 text-left text-zinc-600">
                <tr>
                  ...
                  {matrixPermissions.map((permission) => (
                    <th key={permission} className="min-w-28 px-3 py-2">{permission}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {roleMatrix.map((row) => (
                  <tr key={row.role.id} className="border-t border-zinc-100">
                    <td className="px-3 py-2 font-semibold">{row.role.label}</td>
                    {matrixPermissions.map((permission) => (
                      <td key={permission} className="px-3 py-2">{row.permissions[permission] ? "Si" : "-"}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
