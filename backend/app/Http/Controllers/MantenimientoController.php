...
class MantenimientoController extends Controller
{
    public function dashboard(Request $request)
    {
        ...
        return response()->json([
            'metrics' => [
                ...
                'ordenes_pendientes' => (clone $mantenimientos)->whereIn('estado', ['pendiente', 'en_proceso'])->count(),
    ...
    public function registrarMantenimiento(Request $request)
    {
        $data = $request->validate([
            ...
            'estado' => ['nullable', 'in:pendiente,en_proceso,terminado,cancelado'],
        ...
        $data['estado'] = $data['estado'] ?? (isset($data['fin_at']) ? 'terminado' : 'pendiente');
    ...
    public function actualizarMantenimiento(Request $request, int $id)
    {
        $registro = DB::table('mantenimiento_registros')->find($id);

        if (!$registro) {
            return response()->json(['message' => 'Registro de mantenimiento no encontrado.'], 404);
        }
        ...
        $data = $request->validate([
            ...
            'piezas_retiradas' => ['nullable', 'string'],
            'piezas_instaladas' => ['nullable', 'string'],
            'costo_mano_obra' => ['nullable', 'numeric', 'min:0'],
            'costo_repuestos' => ['nullable', 'numeric', 'min:0'],
            'estado_final' => ['nullable', 'in:operativa,observada,detenida'],
            'recomendacion' => ['nullable', 'string'],
            'proxima_fecha' => ['nullable', 'date'],
        ]);

        $inicio = $data['inicio_at'] ?? $registro->inicio_at;
        $fin = $data['fin_at'] ?? $registro->fin_at;

        if ($fin) {
            $data['duracion_horas'] = max(0, Carbon::parse($inicio)->diffInMinutes(Carbon::parse($fin)) / 60);
            $data['estado'] = $data['estado'] ?? 'terminado';
        }

        $data['updated_at'] = now();

        DB::table('mantenimiento_registros')->where('id', $id)->update($data);

        $fresh = DB::table('mantenimiento_registros')->find($id);
        DB::table('mantenimiento_maquinarias')->where('id', $fresh->maquinaria_id)->update([
            'estado' => $fresh->estado_final === 'detenida' ? 'detenida' : ($fresh->estado === 'en_proceso' ? 'mantenimiento' : 'operativa'),
            'ultimo_mantenimiento' => $fresh->estado === 'terminado' ? now()->toDateString() : DB::raw('ultimo_mantenimiento'),
            'proximo_mantenimiento' => $fresh->proxima_fecha,
            'updated_at' => now(),
        ]);
    ...
    public function actualizarFalla(Request $request, int $id)
    {
        $falla = DB::table('mantenimiento_fallas')->find($id);
        ...
        $data = $request->validate([
            'criticidad' => ['nullable', 'in:baja,media,alta,critica'],
            'estado' => ['nullable', 'in:abierta,en_revision,resuelta,cerrada'],
            'descripcion' => ['nullable', 'string'],
            'accion_realizada' => ['nullable', 'string'],
            'tiempo_inactividad_horas' => ['nullable', 'numeric', 'min:0'],
        ]);

        if (in_array($data['estado'] ?? null, ['resuelta', 'cerrada'], true) && !$falla->resuelta_at) {
            $data['resuelta_at'] = now();
        }

        $data['updated_at'] = now();

        DB::table('mantenimiento_fallas')->where('id', $id)->update($data);

        return response()->json(DB::table('mantenimiento_fallas')->find($id));
    }
