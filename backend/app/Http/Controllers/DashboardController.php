...
class DashboardController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        return match ($role) {
            ...
            'rrhh', 'recursos_humanos' => $this->humanResources(),
    ...
    private function admin()
    {
        return response()->json([
            ...
            'metrics' => [
                ...
                'actividad_reciente' => Venta::whereDate('created_at', now()->toDateString())->count(),
                'estado_servicios' => 1,
                'cantidad_ventas' => Venta::count(),
                'clientes_registrados' => Cliente::where('activo', true)->count(),
                'alertas_sistema' => DB::table('stock_alertas')->where('estado', 'pendiente')->count()
                    + DB::table('mantenimiento_fallas')->whereIn('estado', ['abierta', 'en_revision'])->count(),
            ],
            'chart' => $this->salesChart(Venta::query()),
            'recent_sales' => Venta::with(['cliente', 'vendedor'])->latest()->limit(8)->get(),
            'top_products' => $this->topProducts(),
            'alerts' => [
                ['message' => DB::table('users')->where('is_active', false)->count() . ' usuarios desactivados.'],
                ['message' => DB::table('stock_alertas')->where('estado', 'pendiente')->count() . ' alertas de stock pendientes.'],
                ['message' => DB::table('mantenimiento_fallas')->whereIn('estado', ['abierta', 'en_revision'])->count() . ' fallas abiertas.'],
            ],
        ]);
    }
    ...
    private function management()
    {
        return response()->json([
            ...
            'metrics' => [
                ...
                'ventas_semana' => Venta::whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()])->sum('total'),
                'ventas_mes' => Venta::whereMonth('created_at', now()->month)->sum('total'),
                'total_vendido' => Venta::where('estado', 'cobrada')->sum('total'),
                'ganancia_estimada' => $this->estimatedProfit(Venta::with('detalles.producto')->whereMonth('created_at', now()->month)->get()),
                'ventas_por_efectivo' => DB::table('venta_pagos')->where('metodo', 'efectivo')->whereMonth('created_at', now()->month)->sum('monto'),
                'ventas_por_yape' => DB::table('venta_pagos')->where('metodo', 'yape')->whereMonth('created_at', now()->month)->sum('monto'),
                'ventas_por_transferencia' => DB::table('venta_pagos')->where('metodo', 'transferencia')->whereMonth('created_at', now()->month)->sum('monto'),
                'ventas_por_tarjeta' => DB::table('venta_pagos')->where('metodo', 'tarjeta')->whereMonth('created_at', now()->month)->sum('monto'),
                ...
                'creditos_pendientes' => Credito::whereIn('estado', ['activo', 'vencido'])->where('tipo', 'credito')->sum('saldo_actual'),
                'compras_a_cuenta' => Credito::where('estado', 'activo')->where('tipo', 'cuenta')->sum('saldo_actual'),
                'productos_agotados' => Producto::where('stock', 0)->count(),
                'stock_critico' => Producto::where('stock', '<=', 5)->count(),
                'empleados_activos' => Empleado::where('activo', true)->count(),
                'fallas_criticas' => DB::table('mantenimiento_fallas')->where('criticidad', 'critica')->whereIn('estado', ['abierta', 'en_revision'])->count(),
                'campanas_activas' => DB::table('marketing_campanas')->where('estado', 'activa')->count(),
                'solicitudes_web' => DB::table('public_solicitudes')->count(),
            ],
            'chart' => $this->salesChart(Venta::query()),
            'recent_sales' => Venta::with(['cliente', 'vendedor'])->latest()->limit(8)->get(),
            'top_products' => $this->topProducts(),
