...
class VentaController extends Controller
{
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'numero_operacion' => ['nullable', 'required_if:metodo_pago,yape,transferencia', 'string', 'max:120'],
        ...
        try {
            ...
            [$itemsParaGuardar, $subtotal] = $this->buildItems($data['productos'], $request);
            $descuento = round((float) ($data['descuento'] ?? 0), 2);

            if ($descuento > 0 && !$request->user()->hasPermission('ventas.aprobar')) {
                throw new \Exception('El descuento requiere autorizacion de gerencia o administracion.', 422);
            }
            ...
            if ($canFinalize) {
                ...
                $this->registerDirectPayment($venta, $data, $request);
            }
    ...
    private function buildItems(array $productos, Request $request): array
    {
        $subtotal = 0;
        $itemsParaGuardar = [];
        ...
        foreach ($productos as $item) {
            ...
            if ($precio < (float) $producto->precio && !$request->user()->hasPermission('ventas.aprobar')) {
                throw new \Exception('La reduccion de precio de ' . $producto->nombre . ' requiere autorizacion.', 422);
            }
    ...
    private function registerDirectPayment(Venta $venta, array $data, Request $request): void
    {
        $caja = DB::table('caja_aperturas')
            ->where('responsable_id', $request->user()->id)
            ->where('estado', 'abierta')
            ->latest('abierta_at')
            ->first();

        $paymentId = DB::table('venta_pagos')->insertGetId([
            'venta_id' => $venta->id,
            'caja_apertura_id' => $caja?->id,
            'metodo' => $data['metodo_pago'] === 'mixto' ? 'efectivo' : $data['metodo_pago'],
            'monto' => $venta->total,
            'numero_operacion' => $data['numero_operacion'] ?? null,
            'estado' => in_array($data['metodo_pago'], ['yape', 'transferencia'], true) ? 'pendiente_validacion' : 'validado',
            'validado_por' => in_array($data['metodo_pago'], ['yape', 'transferencia'], true) ? null : $request->user()->id,
            'validado_at' => in_array($data['metodo_pago'], ['yape', 'transferencia'], true) ? null : now(),
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        ...
        if ($caja) {
            DB::table('caja_movimientos')->insert([
                ...
                'venta_id' => $venta->id,
                'tipo' => 'ingreso',
                'metodo' => in_array($data['metodo_pago'], ['credito', 'dinero_cuenta', 'mixto'], true) ? 'otro' : $data['metodo_pago'],
                'monto' => $venta->total,
                'categoria' => 'venta',
                'descripcion' => 'Cobro directo de venta #' . $venta->id . ' / pago #' . $paymentId,
                'registrado_por' => $request->user()->id,
                'created_at' => now(),
                'updated_at' => now(),
            ]);

            $venta->update(['caja_apertura_id' => $caja->id]);
        }
    }
