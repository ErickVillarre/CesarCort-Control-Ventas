...
class DevelopmentDemoDataSeeder extends Seeder
{
    ...
    {
        $referenceProducts = [
            ['nombre' => 'Melamina blanca 18 mm', 'precio' => 200, 'tipo' => 'melamina', 'espesor' => '18mm', 'color' => 'Blanco'],
            ['nombre' => 'Melamina Hispano', 'precio' => 280, 'tipo' => 'melamina', 'espesor' => '18mm', 'color' => 'Hispano'],
            ['nombre' => 'Melamina Pelikano blanca', 'precio' => 300, 'tipo' => 'melamina', 'espesor' => '18mm', 'color' => 'Blanco'],
            ['nombre' => 'Melamina Vesto blanca', 'precio' => 320, 'tipo' => 'melamina', 'espesor' => '18mm', 'color' => 'Blanco'],
            ['nombre' => 'Canto delgado', 'precio' => 1.50, 'tipo' => 'canto', 'canto_tipo' => 'delgado'],
            ['nombre' => 'Canto grueso', 'precio' => 5.00, 'tipo' => 'canto', 'canto_tipo' => 'grueso'],
            ['nombre' => 'Bisagra', 'precio' => 2.00, 'tipo' => 'accesorio'],
            ['nombre' => 'Pistones', 'precio' => 12.00, 'tipo' => 'accesorio'],
            ['nombre' => 'Cortes en escuadradora', 'precio' => 8.00, 'tipo' => 'servicio'],
            ['nombre' => 'Servicio de canteado', 'precio' => 4.00, 'tipo' => 'servicio'],
            ['nombre' => 'Servicio CNC', 'precio' => 35.00, 'tipo' => 'servicio'],
            ['nombre' => 'Instalacion', 'precio' => 80.00, 'tipo' => 'servicio'],
            ['nombre' => 'Retazos de melamina', 'precio' => 15.00, 'tipo' => 'melamina'],
            ['nombre' => 'Accesorios y herrajes', 'precio' => 10.00, 'tipo' => 'accesorio'],
        ];

        foreach ($referenceProducts as $index => $product) {
            if (Producto::where('nombre', $product['nombre'])->exists()) {
                continue;
            }

            Producto::create([
                'codigo' => 'REF-' . str_pad((string) ($index + 1), 3, '0', STR_PAD_LEFT),
                'codigo_barras' => '775100' . str_pad((string) ($index + 1), 6, '0', STR_PAD_LEFT),
                'nombre' => $product['nombre'],
                'precio' => $product['precio'],
                'costo' => round($product['precio'] * 0.65, 2),
                'stock' => 25,
                'tipo' => $product['tipo'],
                'espesor' => $product['espesor'] ?? null,
                'canto_tipo' => $product['canto_tipo'] ?? null,
    ...
    {
        $roles = ['vendedor', 'caja', 'mantenimiento', 'rrhh', 'marketing', 'gerente'];
        ...
        if (DB::table('mantenimiento_registros')->count() < 30) {
            for ($i = 1; $i <= 30; $i++) {
                ...
                DB::table('mantenimiento_registros')->insert([
                    ...
                    'estado' => $i % 9 === 0 ? 'pendiente' : ($i % 7 === 0 ? 'en_proceso' : 'terminado'),
                    'inicio_at' => now()->subDays($i * 2),
                    'fin_at' => $i % 7 === 0 ? null : now()->subDays($i * 2)->addHours(2 + ($i % 4)),
                    'duracion_horas' => $i % 7 === 0 ? 0 : 2 + ($i % 4),
    ...
    private function webRequests(): void
    {
        $products = Producto::limit(15)->get();

        if (DB::table('public_solicitudes')->count() < 15) {
            for ($i = 1; $i <= 15; $i++) {
                DB::table('public_solicitudes')->insert([
                    'nombre' => 'Contacto Web Demo ' . $i,
                    'email' => 'contacto.web' . $i . '@example.com',
                    'telefono' => '98' . str_pad((string) $i, 7, '0', STR_PAD_LEFT),
                    'dni_ruc' => $i % 3 === 0 ? '20' . str_pad((string) (200000000 + $i), 9, '0', STR_PAD_LEFT) : str_pad((string) (76000000 + $i), 8, '0', STR_PAD_LEFT),
                    'direccion' => '[Completar direccion de CESCORT]',
                    'producto' => $products[($i - 1) % max(1, $products->count())]?->nombre,
                    'cantidad_aproximada' => 1 + ($i % 6),
                    'mensaje' => 'Cotizacion o pedido registrado desde landing page demo.',
                    'estado' => $i % 4 === 0 ? 'en_revision' : 'pendiente',
                    'created_at' => now()->subDays($i),
                    'updated_at' => now()->subDays($i),
                ]);
            }
        ...
        if (DB::table('stock_alertas')->where('tipo', 'solicitado_cliente')->count() >= 15) {
            return;
        }

        for ($i = 1; $i <= 15; $i++) {
            DB::table('stock_alertas')->insert([
                'producto_id' => $products[($i - 1) % max(1, $products->count())]?->id,
                'tipo' => 'solicitado_cliente',
                'cantidad_aproximada' => 1 + ($i % 6),
                'observacion' => 'Solicitud web demo #' . str_pad((string) $i, 2, '0', STR_PAD_LEFT) . ': cotizacion o pedido desde landing page.',
                'estado' => $i % 4 === 0 ? 'en_revision' : 'pendiente',
                'created_at' => now()->subDays($i),
                'updated_at' => now()->subDays($i),
            ]);
